...
MODEL_CASCADE = [
    "gemini-3.1-flash-lite-preview",   # stable older model
    "gemini-3.0-flash-preview",        # widely available fallback
]
...
Rules for is_important = true:
- Contains a deadline, meeting, exam, submission, interview, or event with a specific date
- Requires action from the recipient
- Sent from an official/institutional address (iitd.ac.in, university, college, etc.)
- Contains marks, results, attendance, fee notice, or seating arrangement
...
def call_gemini_with_cascade(prompt: str, api_key: str) -> Tuple[str, str]:
    ...
    for model_name in MODEL_CASCADE:
        try:
            ...
            # Safely extract text to avoid 'thought_signature' warnings
...
def analyze_email(
    ...
    prompt = _build_prompt(subject, sender, date, body, infos, student_name, student_entry_no)
